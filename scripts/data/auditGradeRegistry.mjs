 ...
 *
 * ⭐ THE MINNESOTA OSA ENTRY IS THE FIRST **BRANCHING** ONE, added 2026-09-04.
 *
 * It sat unregistered for a week for want of evidence, then for want of a
 * MECHANISM — every other family carries its branch inside the `data_source`
 * string (Florida's `..., audit-reconciled`), and all 21,794 Minnesota rows
 * predate that convention and share ONE bare string. An entry therefore carries
 * an optional `branch(context)` which `gradeFor` consults.
 *
 * ⚠⚠ A BRANCH MAY ONLY RAISE A GRADE ON EVIDENCE. `branch()` returns null when it
 * cannot resolve an entity-year, and the entry's own `value` stands — deliberately
 * the WEAKER branch, the same "mixed source takes the weaker branch" rule that
 * holds CA SCO and Pennsylvania down. The failure mode is UNDER-stating assurance,
 * never over-stating it, and there is no code path that does otherwise.
 *
 * ⚠ Do NOT reach for `branch` when the source string could carry the distinction
 * instead. Florida and Georgia put it in the string precisely so a reader can see
 * it; Minnesota cannot, because relabelling 21,794 historical rows is a bigger and
 * more destructive change than resolving the branch from the publisher's own
...
/**
 * Per-(city, fiscal year) statutory class for Minnesota, read verbatim from the
 * publisher's own `ClassCode` column. Generated by
 * `scripts/buildMnOsaAuditBranch.mjs`; `--check` proves it still matches source.
 *
 * ⚠ readFileSync rather than a JSON import so this module stays loadable under
 * every runner in the repo without an import-attributes flag.
 */
const MN_OSA_BRANCH = JSON.parse(
  readFileSync(new URL('./mnOsaAuditBranch.json', import.meta.url), 'utf8'),
);

/** Fifth class IS "under 2,500 in population" — the OSA report states it. */
const MN_CLASS_NO_AUDIT_DUTY = 5;

/**
 * The audit grade for ONE Minnesota entity-year, or null when it cannot be shown.
 *
 * ⚠⚠ RETURNS null RATHER THAN GUESSING. An entity-year this cannot resolve falls
 * back to the entry's own `value`, which is deliberately the WEAKER branch — the
 * "mixed source takes the weaker branch" rule that already holds CA SCO and
 * Pennsylvania down. Nothing here can ever UPGRADE a row without evidence.
 */
export function mnOsaGradeFor(context) {
  if (!context) return null;
  const { entityType, name, fiscalYear } = context;

  // ⚠ Counties need no lookup: Minn. Stat. § 6.481 subd. 2 makes the annual audit
  // UNCONDITIONAL, so the class does not enter into it.
  if (entityType === 'county') return AUDIT_GRADE.COMPILED_FROM_AUDITED;
  if (entityType !== 'city') return null;

  const classCode = MN_OSA_BRANCH.cities?.[name]?.[String(fiscalYear)];
  if (!Number.isInteger(classCode)) return null;
  return classCode === MN_CLASS_NO_AUDIT_DUTY
    ? AUDIT_GRADE.SELF_REPORTED_UNAUDITED
    : AUDIT_GRADE.COMPILED_FROM_AUDITED;
}
...
export const AUDIT_GRADE_REGISTRY = [
  ...
  {
    // ⚠⚠ THE FIRST BRANCHING ENTRY IN THIS REGISTRY, and the reason is that
    // Minnesota is genuinely TWO families sharing one publisher and one string.
    //
    //   counties            § 6.481 subd. 2 — "A county must have an annual
    //                       financial audit."            -> compiled_from_audited
    //   cities over 2,500   § 471.697(c) — must "Submit to the state auditor
    //                       audited financial statements ... attested to by a
    //                       certified public accountant, public accountant, or
    //                       the state auditor."          -> compiled_from_audited
    //   cities under 2,500  § 471.698 — a statement "in the style and form
    //                       prescribed by the state auditor". NO AUDIT CLAUSE,
    //                       and 56.1% of them were CASH basis in 2023.
    //                                                    -> self_reported_unaudited
    //
    // ⚠ Every other family carries its branch inside the `data_source` string
    // (Florida's `..., audit-reconciled`). All 21,794 Minnesota rows predate that
    // convention and share ONE bare string, so `branch` resolves it from the
    // entity instead. See scripts/data/mnOsaAuditBranch.json.
    //
    // ⚠⚠ `value` IS THE WEAKER BRANCH ON PURPOSE. It is what stands whenever
    // `branch()` cannot resolve an entity-year — the "mixed source takes the
    // weaker branch" rule that already holds CA SCO and Pennsylvania down. No
    // path in `gradeFor` can raise a row above this without the publisher's own
    // ClassCode saying so, so the failure mode is UNDER-stating assurance, never
    // over-stating it.
    id: 'mn-osa-lgf',
    match: /^Minnesota Office of the State Auditor City\/County Finances Report$/,
    value: AUDIT_GRADE.SELF_REPORTED_UNAUDITED,
    branch: mnOsaGradeFor,
    evidence: {
      document:
        'Minnesota Office of the State Auditor, 2023 City Finances Report and 2022 County '
        + 'Finances Report (Scope and Methodology sections), the Local Government Financial '
        + 'Reporting Form Instructions (updated February 2026), and Minn. Stat. §§ 6.481, '
        + '471.697 and 471.698, all read 2026-09-04. '
        ...
        + '⚠⚠ A MIXED SOURCE resolved per entity-year, so this entry BRANCHES rather '
        + 'than asserting one grade for the family. ⚠ § 6.48, cited by older notes, was '
        + 'REPEALED in 2015 — the live county cite is § 6.481.',
      figures:
        'WHAT THE DATA IS, verbatim (2023 City Finances Report, Scope and Methodology): the '
        + 'report summarises "the financial operations of the 851 Minnesota cities that '
        + 'PROVIDED THEIR FINANCIAL INFORMATION to the Office of the State Auditor", which are '
        + '"required to submit annual financial reports to the OSA" pursuant to Minn. Stat. '
        + '§ 6.74 and §§ 471.697-.698. ⚠ Stopping there grades the whole '
        + 'family self_reported_unaudited — the Florida trap — because it ignores what '
        + 'the law requires to ACCOMPANY the form. '
        + 'THE AUDIT DUTY SPLITS BY CLASS, verbatim: § 6.481 subd. 2 "A county must have an '
        + 'annual financial audit."; § 471.697(c) cities over 2,500 must "Submit to the '
        + 'state auditor audited financial statements which have been attested to by a certified '
        + 'public accountant, public accountant, or the state auditor"; § 471.698 requires '
        + 'of cities under 2,500 only "a detailed statement of the financial affairs of the city '
        + '... in the style and form prescribed by the state auditor" — NO AUDIT CLAUSE, and '
        + 'the report adds that 347 of 619 such cities (56.1%) used a CASH basis in 2023. '
        + 'OSA TIES THE FORM TO THE AUDIT IN ITS OWN WORDS, TWICE. Form Instructions, Section 10: '
        + '"If supporting financial statements/audit do not match to the amounts in the Form, '
        + 'please provide an explanation as to the differences. The Form must be accurate." And '
        + 'the County Finances Report NAMES ITS EXCEPTION: "Kittson County submitted their 2022 '
        + 'Annual Financial Reporting Form without an accompanying audit. Unaudited data provided '
        + 'by Kittson County is included in this report for comparison purposes." A rule stated '
        + 'by naming its exception is stronger than one stated abstractly. '
        + '⚠ WEAKER THAN FLORIDA, where DFS ITSELF reconciles the AFR to the audited '
        + 'statements and RETURNS it if it does not tie; Minnesota puts that duty on the '
        + 'submitter and OSA "reviews". STRONGER THAN OHIO AOS, which prints "UNAUDITED" on its '
        + 'own download page. '
        + 'THE BRANCH IS THE PUBLISHER’S OWN, not a threshold computed here: the raw '
        + 'data’s ClassCode column carries the § 410.01 class and reconciles exactly '
        + 'with the report’s prose — FY2023 holds 619 rows at class 5 and the report '
        + 'says "347 of the 619 small cities (56.1 percent)". Duluth is class 1 at a population '
        + 'of 86,788, confirming § 410.01’s 25% retention rule rather than inferring '
        + 'it. ⚠ Population is NOT the rule (class comes from the DECENNIAL census, which is '
        + 'why the distribution steps at FY2020) and ⚠ GAAPInd is basis of accounting, NOT '
        + 'audit status.',
    },
  },
];
 ...
 */
export function gradeFor(dataSource, context = null) {
  const base = classifyAxis(dataSource, AUDIT_GRADE_REGISTRY, AUDIT_GRADE_VALUES, AUDIT_GRADE.UNKNOWN);
  if (base.entryId === null) return base;

  const entry = AUDIT_GRADE_REGISTRY.find((e) => e.id === base.entryId);
  if (typeof entry?.branch !== 'function') return base;

  // ⚠⚠ A BRANCHING ENTRY MAY ONLY MOVE THE GRADE ON EVIDENCE. `branch()` returns
  // null when it cannot resolve the entity-year, and the entry's own `value` —
  // the weaker branch — stands. There is no path here that grades a row higher
  // than the family's floor without the publisher's own classification saying so.
  const branched = entry.branch(context);
  if (branched == null) return base;
  if (!AUDIT_GRADE_VALUES.includes(branched) || branched === AUDIT_GRADE.UNKNOWN) return base;
  return { value: branched, entryId: base.entryId };
}
