...
export const BASIS_REGISTRY = [
  ...
  },
  {
    ...
    // FEDERAL FILING gap, not evidence the government published no ACFR, and it
    // is never written as $0. See SC_CITY_COVERAGE_GAPS.
    //
  ...
  },
  {
    ...
    evidence: {
      document: 'Marion County FY2016-FY2025, Hamilton County FY2016-FY2025, Allen County '
              + 'FY2016-FY2024, Lake County FY2020-FY2021, St. Joseph County FY2019-FY2022 and '
              + 'FY2024-FY2025, Elkhart / Tippecanoe / Hendricks / Vanderburgh / Johnson / '
              + 'Monroe County FY2019-FY2024 and Porter County FY2019-FY2021 and FY2023-FY2024 '
              + 'audited ACFRs — closed calendar years, every one of them. '
              ...
              + 'the FAC fiscal-year census independently returns month 1 for all twelve '
              + 'counties, checked per entity-YEAR by censusGuard at load time so a '
              + 'contradiction fails the load. ⭐ All 79 GAAP entity-years come back ACTIVELY '
              + 'CONFIRMED; none rests on the census being silent.',
      figures: 'Every stored figure is the printed TOTAL GOVERNMENTAL FUNDS column of the '
             ...
             + 'Balances — a year-end GAAP actual, tying at exactly $0 on both sides in 155 of '
             + 'the 156 extractions, the 156th being Porter FY2020 expenditure at a REGISTERED '
             + 'EXACT delta of -1 that is the county\'s own footing (see the fund-scope entry). '
             + 'NOT an appropriation. '
             + '⚠⚠ EVERY ISSUER ALSO PRINTS A BUDGETARY COMPARISON SCHEDULE IN THE SAME '
             + 'DOCUMENT, which IS budget basis and would carry no arithmetic symptom under a '
             + 'GAAP-actual label. What keeps it out is the reader excluding any page carrying '
             + '"budgetary" or "budget and actual", the earliest-qualifying-page rule, the tie '
             + 'gate and the root-label check. '
             + '⚠⚠ CORRECTION TO WHAT THIS ENTRY SAID THROUGH WAVE 2: a wrapper\'s '
             + '`statement_anchor` does NOT "require the words Governmental Funds". '
             + '`find_statement_page` ORs the anchor with the title regex, so an anchor can only '
             + 'WIDEN which pages qualify — measured over all 64 wave-1/2 Indiana documents, the '
             + 'anchor moves ZERO pages. The General-Fund-only statement Marion prints on p.114 '
             + 'is excluded by the earliest-page rule and the root-label check, not by an anchor. '
             + 'Wave 3\'s four wrappers declare no anchor at all. '
