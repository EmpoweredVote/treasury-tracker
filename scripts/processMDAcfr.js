#!/usr/bin/env node
/**
 * Maryland General Fund Operating (Expenditure) Loader — FY2022-FY2025 ACTUAL
...
const dataSource = (fy) => `Maryland State ACFR — General Fund (FY${fy} actual, GAAP basis)`;
...
function buildTree(fy, ex) {
  ...
  return { jsonTree: [{ n: 'Maryland General Fund Budget', a: total * UNITS, c: children }], total: total * UNITS, rowCount: children.length };
}
async function main() {
  ...
  if (!dryRun) {
    ...
    const srcPayload = { name: 'Maryland General Fund Operating Budget', api_type: 'pdf_download', dataset_type: 'operating', dataset_id: 'md-acfr-gf-operating', base_url: 'https://www.marylandcomptroller.gov/reports/annual-comprehensive-financial-report-acfr.html', fiscal_years: years, municipality_id: muniId };
