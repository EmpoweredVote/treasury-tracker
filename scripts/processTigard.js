#!/usr/bin/env node
/**
 * Tigard, OR Budget Loader - General Fund operating (expenditure-by-function)
 * + revenue (revenue-by-source), FY2022-FY2025, ACTUAL (ACFR GAAP basis).
 *
 * Consumes `scripts/extractTigard.py` (a thin wrapper over lib/acfrGF.py) and
 ...
 *
 * Source: City of Tigard Annual Comprehensive Financial Report, GAAP actuals,
 ...
 *
 * Tigard's expenditure section is FLAT -- no 'Current:' grouping, every function
 * at one level -- so the operating tree is ONE level deep and the icicle has no
 * drill-down for Tigard (the same accepted limitation as the flat-source states).
 * Tigard also titles its statement 'CHANGES IN FUND BALANCE' (singular), which
 * the shared _TITLE pattern now accommodates.
 *
 * SOURCE DISCOVERY: tigard-or.gov returns 403 to curl for GET and HEAD regardless
 * of headers (TLS fingerprinting). PDFs were fetched through a real browser via
 * scripts/fetchViaBrowser.mjs. Tigard's document links carry no fiscal year, only
 * opaque ids, so each file's FY was confirmed from the document itself -- the
 * loader re-asserts that match per FY before writing.
 *
 * SCOPE: GF-only. Window starts at FY2022: the finance page links four ACFRs,
 * with older reports behind a separate public-records portal.
 *
 ...
 *
 * Usage:
 *   node scripts/processTigard.js --dry-run            # operating dry-run, all FYs
 *   node scripts/processTigard.js --revenue --dry-run  # revenue dry-run, all FYs
 *   node scripts/processTigard.js                      # LIVE operating, all FYs
 *   node scripts/processTigard.js --revenue            # LIVE revenue, all FYs
 *
 * Requires: `pdftotext -table` (poppler) on PATH; Tigard seeded via
...
// docs/Tigard/*.pdf is gitignored (docs/*); a worktree can't see it -- fall back
// to the main working tree's docs/Tigard/ via git-common-dir.
function resolvePdfDir() {
  const candidate = path.join(ROOT, 'docs', 'Tigard');
  ...
  try {
    ...
    const mainCandidate = path.join(mainRoot, 'docs', 'Tigard');
...
function discoverPdfsByFY(pdfDir) {
  ...
  for (const f of files) {
    const m = f.match(/^tigard-(\d{4})-acfr\.pdf$/i);
...
const POPULATION = 57301; // Census PEP vintage 2024, matches the seeder
const SANITY_MAX = 2_000_000_000;

// Durable per-FY source PDF URLs, stamped onto every loaded budgets row
// (verified 2026-07-27).
const URLS = {
  2025: 'https://www.tigard-or.gov/home/showpublisheddocument/6307/639051272038830000',
  2024: 'https://www.tigard-or.gov/home/showpublisheddocument/5021/638712267566470000',
  2023: 'https://www.tigard-or.gov/home/showpublisheddocument/4240/638481922257230000',
  2022: 'https://www.tigard-or.gov/home/showpublisheddocument/3113/638210414060600000',
};

// ── Run the Python extractor, return parsed JSON (or throw) ───────────────────
function extractPDF(pdfPath, mode) {
  const pyScript = path.join(ROOT, 'scripts', 'extractTigard.py');
  ...
  if (result.status !== 0) {
    throw new Error(
      `extractTigard.py failed (exit ${result.status}) for ${pdfPath} [--mode ${mode}]: ` +
...
function dataSourceLabel(fy, datasetType) {
  ...
  return `City of Tigard ACFR — General Fund ${kind} (FY${fy} actual, GAAP basis)`;
}

// ── Resolve Tigard's municipality_id; refuse to write if not found ─────────────
async function ensureMunicipality(supabase) {
  ...
  if (error) { console.error('  ERROR resolving Tigard municipality:', error.message); process.exit(2); }
  if (!data?.id) {
    console.error('  Tigard, OR (entity_type=city) municipality not found — run ' +
                  'scripts/seedWashingtonCountyOregonCities.js first');
    process.exit(2);
  }
  console.log(`  Municipality: Tigard, OR (${data.id})`);
  return data;
}

// ── Ephemeral data_sources lifecycle (WR-05/LOAD-01) ─────────────────────────
async function createEphemeralDataSource(supabase, muniId, datasetType) {
  const datasetId = datasetType === 'revenue' ? 'tigard-acfr-gf-revenue' : 'tigard-acfr-gf-operating';
  const kind = datasetType === 'revenue' ? 'Revenue' : 'Operating';
  const payload = {
    name: `Tigard General Fund ${kind} Budget`,
    api_type: 'pdf_download',
    dataset_type: datasetType,
    dataset_id: datasetId,
    base_url: 'https://www.tigard-or.gov/your-government/departments/finance',
    fiscal_years: FYS,
...
async function processMode(supabase, muniId, dryRun, mode, targetFY, pdfsByFY) {
  ...
  try {
    for (const fy of years) {
      ...
      if (!pdfPath) throw new Error(`No PDF found for FY${fy} in docs/Tigard/ — aborting`);
...
async function main() {
  ...
  if (!pdfsByFY.size) {
    console.error(`No Tigard ACFR PDFs found in ${pdfDir}`);
    process.exit(1);
  }

  console.log(`Tigard Budget Loader${dryRun ? ' (dry-run)' : ''} [${mode}]`);
