#!/usr/bin/env node
/**
 * Tennessee General Fund Revenue Loader — FY2022-2026
 * Source: TN Dept of Finance and Administration (tn.gov/finance)
 * TN fiscal year ends June 30. NO state income tax (Hall Tax on investment income
 * was fully phased out by Jan 2021). Revenue dominated by Sales Tax.
 * Confidence: estimated for all years.
 *
 * Usage:
 *   node scripts/processTNRevenue.js [--dry-run] [--fy YYYY]
 */
...
const REVENUE = {
  2022: { total: 14_500_000_000, confidence: 'estimated', categories: [
    { name: 'Sales and Use Tax', total: 8_000_000_000, lineItems: [{ name: 'Sales and Use Tax', amount: 8_000_000_000 }] },
    { name: 'Franchise and Excise Tax', total: 2_200_000_000, lineItems: [{ name: 'Franchise and Excise Tax (business)', amount: 2_200_000_000 }] },
    { name: 'Tobacco and Alcohol Tax', total: 500_000_000, lineItems: [{ name: 'Tobacco and Alcohol Tax', amount: 500_000_000 }] },
    { name: 'Other Taxes and Revenues', total: 3_800_000_000, lineItems: [
      { name: 'Motor Vehicle Registration', amount: 800_000_000 },
      { name: 'Insurance Premiums Tax', amount: 600_000_000 },
      { name: 'Other Taxes and Fees', amount: 2_400_000_000 },
    ]},
  ]},
  2023: { total: 16_000_000_000, confidence: 'estimated', categories: [
    ...
    { name: 'Franchise and Excise Tax', total: 2_500_000_000, lineItems: [{ name: 'Franchise and Excise Tax (business)', amount: 2_500_000_000 }] },
    { name: 'Tobacco and Alcohol Tax', total: 500_000_000, lineItems: [{ name: 'Tobacco and Alcohol Tax', amount: 500_000_000 }] },
    { name: 'Other Taxes and Revenues', total: 4_000_000_000, lineItems: [
      { name: 'Motor Vehicle Registration', amount: 900_000_000 },
...
function buildTree(fy) {
  ...
  return { jsonTree: [{ n: 'Tennessee General Fund Revenue', a: total, c: children }], total, rowCount: categories.length };
}
...
async function main() {
  ...
  let ds;
  if (!dryRun) {
    const srcPayload = { name: `${STATE_NAME} General Fund Revenue`, api_type: 'html', dataset_type: 'revenue', dataset_id: 'tn-gf-revenue', base_url: 'https://www.tn.gov/finance/budget.html', fiscal_years: [2022,2023,2024,2025,2026], municipality_id: muniId };
