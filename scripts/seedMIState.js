#!/usr/bin/env node
/**
 * Michigan State Seeder
 *
 * Performs the following (all idempotent):
 *   A. Upsert Michigan municipality row (entity_type='state', population=10077331,
 *      population_year=2024, state='MI')
 *   B. Upsert two data_source rows:
 *        - 'Michigan General Fund Operating Budget'
 ...
 *
 * Michigan is a state-level entity (entity_type='state').
 ...
 *
 * Data sources:
 *   - SFA State Budget Overview (Dec 2025): GF/GP appropriations by department
 *   - SFA Economic Outlook: GF/GP revenue by tax type
 *   - SFA Revenue History: Historical GF/GP revenue totals FY1980-present
 *   - May 2026 CREC: Consensus revenue estimates
 *
 * Usage:
 *   node scripts/seedMIState.js
 *
 * Env vars:
...
// county_id stays NULL — states don't belong to a county.
// Population: 2024 US Census estimate for Michigan.
const MICHIGAN = {
  name:            'Michigan',
  state:           'MI',
  entity_type:     'state',
...
// Two canonical rows (not per-FY). municipality_id injected after upsert.
// fiscal_years = [2022,2023,2024,2025,2026] = FY2021-22 through FY2025-26.
// Michigan fiscal year ends September 30.
...
// ── Main ─────────────────────────────────────────────────────────────────────
async function main() {
  console.log('Seeding Michigan state — municipality + data_sources...\n');

  // ── Step A: Upsert Michigan state municipality ────────────────────────────
  console.log(`Upserting municipality: ${MICHIGAN.name}, ${MICHIGAN.state} (entity_type=${MICHIGAN.entity_type})`);
  const miStateId = await upsertMunicipality(MICHIGAN);
  console.log(`  id: ${miStateId}\n`);

  // ── Step B: Upsert data_source rows ──────────────────────────────────────
  const dataSources = DATA_SOURCE_TEMPLATES.map(t => ({ ...t, municipality_id: miStateId }));
