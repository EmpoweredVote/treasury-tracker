#!/usr/bin/env node
/**
 * Pennsylvania State Seeder
 *
 * Performs the following (all idempotent):
 *   A. Upsert Pennsylvania municipality row (entity_type='state', population=13002700,
 *      population_year=2024, state='PA')
 *   B. Upsert two data_source rows:
 *        - 'Pennsylvania General Fund Operating Budget'
 ...
 *        - 'Pennsylvania General Fund Revenue'
...
// county_id stays NULL — states don't belong to a county.
// Population: 2024 Census estimate (~13.0M PA total resident population).
const PENNSYLVANIA = {
  name:            'Pennsylvania',
  state:           'PA',
  entity_type:     'state',
...
// ── Main ─────────────────────────────────────────────────────────────────────
async function main() {
  console.log('Seeding Pennsylvania state — municipality + data_source...\n');

  // ── Step A: Upsert Pennsylvania state municipality ────────────────────────
  console.log(`Upserting municipality: ${PENNSYLVANIA.name}, ${PENNSYLVANIA.state} (entity_type=${PENNSYLVANIA.entity_type})`);
  const paStateId = await upsertMunicipality(PENNSYLVANIA);
  console.log(`  id: ${paStateId}\n`);

  // ── Step B: Upsert data_source rows ──────────────────────────────────────
  const dataSources = DATA_SOURCE_TEMPLATES.map(t => ({ ...t, municipality_id: paStateId }));
