#!/usr/bin/env node
/**
 * Connecticut General Fund Revenue (by source) Loader — FY2002-FY2025 ACTUAL (23 yrs; FY2006 hole)
...
const dataSource = (fy) => `Connecticut State ACFR — General Fund Revenue (FY${fy} actual, GAAP basis)`;
...
async function main() {
  ...
  for (const fy of years) {
    ...
    if (Math.abs(diff) > TOL) { console.log(`FY${fy}: SKIP (rev sum ${catSum} ≠ ${ex.expTotal}, diff ${diff}) — honest hole`); holes.push(fy); continue; }
