...
Builds the General Fund trees from a City of Sherwood Annual Comprehensive
...
Changes in Fund Balances*:
  ...
  --mode operating  2-level GF expenditure-by-function tree
                    (Current -> functions; Noncurrent -> capital/debt service)

Same contract as scripts/extractBend.py and scripts/extractTucson.py:
...
WHY THIS IS NOT extractBend.py
------------------------------
Sherwood's statement differs structurally from Bend's in two ways that a shared
parser cannot infer from the text alone:

1. **Everything is UPPERCASE** — "TOTAL REVENUES", "TOTAL EXPENDITURES",
   "REVENUES:", "EXPENDITURES:". Bend's page-qualification tests are
   case-sensitive, so a Sherwood ACFR fails detection outright ("primary GF
   statement not found") rather than mis-parsing. All matching here is
   case-insensitive.

2. **Capital Outlay is a CHILD, not a root leaf.** Sherwood groups spending as
   `Current:` (Administration, Community Development, Public Safety, Community
   Services, Public Works) then `Noncurrent` (Capital Outlay, Debt Service -
   Principal, Debt Service - Interest). Bend puts Capital outlay at the root,
   after its Debt service parent. `-table` output is flush-left for both, so
   indentation cannot distinguish them — the grouping has to be encoded per city.

Sherwood shares Bend's dash-zero convention: a `-` marks a $0 fund cell. Such a
row is DATA (GF = $0), not a wrapped label. Getting this wrong glues the label
onto the next row while leaving amounts — and therefore `tie_delta` — correct,
so the corruption is silent. $0 rows are dropped from the emitted tree but
reported in `zero_rows` so the drop stays auditable.
...
# ── Tree builders ────────────────────────────────────────────────────────────
# Sherwood's two expenditure groupings. "Debt service" is included defensively
# in case an earlier ACFR splits it out as its own parent.
_PARENTS = ('current', 'noncurrent', 'debt service')
...
def build_operating(lines, col_anchors):
    """2-level tree. Every row belongs to the most recent parent header; unlike
    Bend there is NO root-level capital-outlay leaf (Sherwood files Capital
    Outlay under Noncurrent)."""
...
if __name__ == '__main__':
    ap = argparse.ArgumentParser(description='Sherwood, OR ACFR General Fund extractor')
    ap.add_argument('pdf_path', help='Path to a City of Sherwood ACFR PDF')
