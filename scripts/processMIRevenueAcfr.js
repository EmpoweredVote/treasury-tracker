#!/usr/bin/env node
/**
 * Michigan General Fund Revenue (by source) Loader — FY2019-FY2025 ACTUAL (7 yrs)
 ...
 *
 * Phase 109-05 (ACFR-18 / ACFR-19 / ACFR-20 / RECON-08). Revenue is NEW on the MI state node
 *   (38c9f1ff) → pure insert keyed (muni,fy,'revenue'); enables "Money In". Node by name='Michigan'.
 *
 ...
 * Usage: node scripts/processMIRevenueAcfr.js [--dry-run] [--fy YYYY]
 */
...
const dataSource = (fy) => `Michigan State ACFR — General Fund Revenue (FY${fy} actual, GAAP basis)`;
...
// for form's sake. Gated per-dataset (rev tie here).
...
function buildTree(fy, ex) {
  ...
  return { jsonTree: [{ n: 'Michigan General Fund Revenue', a: total * UNITS, c: children }], total: total * UNITS, rowCount: children.length };
}
async function main() {
  const { values: opts } = parseArgs({ options: { 'dry-run': { type: 'boolean', default: false }, fy: { type: 'string' } }, strict: false });
  const dryRun = opts['dry-run']; const targetFY = opts.fy ? parseInt(opts.fy, 10) : null;
  const years = targetFY ? [targetFY] : YEARS;
  console.log(`${STATE_NAME} GF Revenue Loader (ACFR GAAP, parser, thousands×${UNITS}, Sep-30 FY-end)${dryRun ? ' (dry-run)' : ''}\n`);
  ...
  if (!dryRun) {
    ...
    const srcPayload = { name: 'Michigan General Fund Revenue', api_type: 'pdf_download', dataset_type: 'revenue', dataset_id: 'mi-acfr-gf-revenue', base_url: 'https://www.michigan.gov/budget/fiscal-pages/reports/annual-comprehensive-financial-report', fiscal_years: years, municipality_id: muniId, fiscal_year_start_month: 10 };
