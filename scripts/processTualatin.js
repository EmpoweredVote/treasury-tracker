#!/usr/bin/env node
/**
 * Tualatin, OR Budget Loader - General Fund operating (expenditure-by-function)
 * + revenue (revenue-by-source), FY2021-FY2025, ACTUAL (ACFR GAAP basis).
 *
 * Consumes `scripts/extractTualatin.py` and loads exclusively through the
 ...
 *
 * Source: City of Tualatin Annual Comprehensive Financial Report, GAAP actuals,
 ...
 *
 * Tualatin files Capital outlay as a ROOT-LEVEL PEER of Current and Debt service
 * (the GASB convention), verified against `pdftotext -layout` indentation.
 * FY2021-FY2024 report $0 GF debt service, so that parent is legitimately
 * absent from those years' trees rather than being an extraction gap.
 *
 ...
 *
 * Safety: args-array spawnSync (no shell); controlled docs/Tualatin/ readdir;
 ...
 *
 * Usage:
 *   node scripts/processTualatin.js --dry-run            # operating dry-run, all FYs
 *   node scripts/processTualatin.js --revenue --dry-run  # revenue dry-run, all FYs
 *   node scripts/processTualatin.js                      # LIVE operating, all FYs
 *   node scripts/processTualatin.js --revenue            # LIVE revenue, all FYs
 *   node scripts/processTualatin.js --fy 2025            # single FY
 *
 * Requires: `pdftotext -table` (poppler) on PATH; Tualatin seeded via
...
// docs/Tualatin/*.pdf is gitignored (docs/*); a worktree can't see it -- fall back
// to the main working tree's docs/Tualatin/ via git-common-dir.
function resolvePdfDir() {
  const candidate = path.join(ROOT, 'docs', 'Tualatin');
  ...
  try {
    ...
    const mainCandidate = path.join(mainRoot, 'docs', 'Tualatin');
...
function discoverPdfsByFY(pdfDir) {
  ...
  for (const f of files) {
    const m = f.match(/^tualatin-(\d{4})-acfr\.pdf$/i);
...
const POPULATION = 28036; // Census PEP vintage 2024, matches the seeder
const SANITY_MAX = 2_000_000_000;

// Durable per-FY source PDF URLs, stamped onto every loaded budgets row
// (verified 2026-07-27).
const URLS = {
  2025: 'https://tualatinoregon.gov/app/uploads/2026/06/City-of-Tualatin-Annual-Comprehensive-Financial-Report-FY25.pdf',
  2024: 'https://tualatinoregon.gov/app/uploads/2025/12/city_of_tualatin_annual_comprehensive_financial_report_-_fy24.pdf',
  2023: 'https://tualatinoregon.gov/app/uploads/2025/12/city_of_tualatin_acfr_-_fy23_final.pdf',
  2022: 'https://tualatinoregon.gov/app/uploads/2025/12/city_of_tualatin_annual_comprehensive_financial_report_-_fy2022.pdf',
  2021: 'https://tualatinoregon.gov/app/uploads/2025/12/city_of_tualatin_-_annual_comprehensive_financial_report_-_fy21.pdf',
};

// ── Run the Python extractor, return parsed JSON (or throw) ───────────────────
function extractPDF(pdfPath, mode) {
  const pyScript = path.join(ROOT, 'scripts', 'extractTualatin.py');
  ...
  if (result.status !== 0) {
    throw new Error(
      `extractTualatin.py failed (exit ${result.status}) for ${pdfPath} [--mode ${mode}]: ` +
...
function dataSourceLabel(fy, datasetType) {
  ...
  return `City of Tualatin ACFR — General Fund ${kind} (FY${fy} actual, GAAP basis)`;
}

// ── Resolve Tualatin's municipality_id; refuse to write if not found ─────────────
async function ensureMunicipality(supabase) {
  ...
  if (error) { console.error('  ERROR resolving Tualatin municipality:', error.message); process.exit(2); }
  if (!data?.id) {
    console.error('  Tualatin, OR (entity_type=city) municipality not found — run ' +
                  'scripts/seedWashingtonCountyOregonCities.js first');
    process.exit(2);
  }
  console.log(`  Municipality: Tualatin, OR (${data.id})`);
  return data;
}

// ── Ephemeral data_sources lifecycle (WR-05/LOAD-01) ─────────────────────────
async function createEphemeralDataSource(supabase, muniId, datasetType) {
  const datasetId = datasetType === 'revenue' ? 'tualatin-acfr-gf-revenue' : 'tualatin-acfr-gf-operating';
  const kind = datasetType === 'revenue' ? 'Revenue' : 'Operating';
  const payload = {
    name: `Tualatin General Fund ${kind} Budget`,
    api_type: 'pdf_download',
    dataset_type: datasetType,
    dataset_id: datasetId,
    base_url: 'https://tualatinoregon.gov/internal-services-departments/finance/financial-reports/',
    fiscal_years: FYS,
...
async function processMode(supabase, muniId, dryRun, mode, targetFY, pdfsByFY) {
  ...
  try {
    for (const fy of years) {
      ...
      if (!pdfPath) throw new Error(`No PDF found for FY${fy} in docs/Tualatin/ — aborting`);
...
async function main() {
  ...
  if (!pdfsByFY.size) {
    console.error(`No Tualatin ACFR PDFs found in ${pdfDir}`);
    process.exit(1);
  }

  console.log(`Tualatin Budget Loader${dryRun ? ' (dry-run)' : ''} [${mode}]`);
