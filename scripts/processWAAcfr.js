#!/usr/bin/env node
/**
 * Washington General Fund Operating (Expenditure) Loader — FY2020-FY2025 ACTUAL (6 yrs)
 ...
 *
 * Phase 109-04 (ACFR-17 / ACFR-19 / ACFR-20 / RECON-08). Replaces the NASBO operating rows on the
 *   WA state node (d8257751-45c4-4853-9621-e1841e7d4998) in place. Node by name='Washington'.
 *
 ...
 * Usage: node scripts/processWAAcfr.js [--dry-run] [--fy YYYY]
 */
...
const dataSource = (fy) => `Washington State ACFR — General Fund (FY${fy} actual, GAAP basis)`;
...
function buildTree(fy, ex) {
  ...
  return { jsonTree: [{ n: 'Washington General Fund Budget', a: total * UNITS, c: children }], total: total * UNITS, rowCount: children.length };
}
async function main() {
  ...
  if (!dryRun) {
    ...
    const srcPayload = { name: 'Washington General Fund Operating Budget', api_type: 'pdf_download', dataset_type: 'operating', dataset_id: 'wa-acfr-gf-operating', base_url: 'https://ofm.wa.gov/spending/financial-audit-reports/annual-comprehensive-financial-report/', fiscal_years: years, municipality_id: muniId };
