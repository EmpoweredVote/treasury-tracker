#!/usr/bin/env node
/**
 * Florida State Seeder
 *
 * Performs the following (all idempotent):
 *   A. Upsert Florida municipality row (entity_type='state', population=21538187,
 *      population_year=2024, state='FL')
 *   B. Upsert two data_source rows:
 *        - 'Florida General Fund Operating Budget'
...
// county_id stays NULL — states don't belong to a county.
// Population: 2024 Census estimate (~21.5M FL total resident population).
const FLORIDA = {
  name:            'Florida',
  state:           'FL',
  entity_type:     'state',
...
// fiscal_years = [2022,2023,2024,2025,2026] = FY2021-22 through FY2025-26.
// Florida General Revenue Fund (GRF) — the primary state operating fund.
// No personal income tax in Florida; GRF is ~40-50% of total appropriations.
...
// ── Main ─────────────────────────────────────────────────────────────────────
async function main() {
  console.log('Seeding Florida state — municipality + data_source...\n');

  // ── Step A: Upsert Florida state municipality ─────────────────────────────
  console.log(`Upserting municipality: ${FLORIDA.name}, ${FLORIDA.state} (entity_type=${FLORIDA.entity_type})`);
  const flStateId = await upsertMunicipality(FLORIDA);
  console.log(`  id: ${flStateId}\n`);

  // ── Step B: Upsert data_source rows ──────────────────────────────────────
  const dataSources = DATA_SOURCE_TEMPLATES.map(t => ({ ...t, municipality_id: flStateId }));
