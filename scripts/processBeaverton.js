#!/usr/bin/env node
/**
 * Beaverton, OR Budget Loader - General Fund operating (expenditure-by-function)
 * + revenue (revenue-by-source), FY2021-FY2025, ACTUAL (ACFR GAAP basis).
 *
 * Consumes `scripts/extractBeaverton.py` (a thin wrapper over lib/acfrGF.py) and
 ...
 *
 * Source: City of Beaverton Annual Comprehensive Financial Report, GAAP actuals,
 ...
 *
 * Beaverton files Capital Outlay as a ROOT-LEVEL PEER of Current and Debt
 * service, verified against `pdftotext -layout` indentation. FY2021-FY2023
 * report $0 GF debt service, so that parent is legitimately absent from those
 * years' trees rather than being an extraction gap.
 *
 * SOURCE DISCOVERY: Beaverton runs CivicPlus Evolve and injects its document
 * links client-side -- a plain HTTP fetch of /<year>-financial-audit returns a
 * page with NO document links. These URLs were recovered by rendering each page
 * with the cached Playwright Chromium in headless --dump-dom mode. The PDFs sit
 * on TWO different hosts by vintage (content.civicplus.com/api/assets/... for
 * FY2021-FY2024, beavertonoregon.gov/asset/... for FY2025), and the FY2024 URL
 * needs its ?scope=all query string, so every URL is pinned literally below
 * rather than pattern-built.
 *
 * SCOPE: GF-only, consistent with the other Oregon city loads. Beaverton has the
 * richest revenue tree of the four (12-13 sources).
 *
 ...
 *
 * Safety: args-array spawnSync (no shell); controlled docs/Beaverton/ readdir;
 ...
 *
 * Usage:
 *   node scripts/processBeaverton.js --dry-run            # operating dry-run, all FYs
 *   node scripts/processBeaverton.js --revenue --dry-run  # revenue dry-run, all FYs
 *   node scripts/processBeaverton.js                      # LIVE operating, all FYs
 *   node scripts/processBeaverton.js --revenue            # LIVE revenue, all FYs
 *   node scripts/processBeaverton.js --fy 2025            # single FY
 *
 * Requires: `pdftotext -table` (poppler) on PATH; Beaverton seeded via
...
// docs/Beaverton/*.pdf is gitignored (docs/*); a worktree can't see it -- fall back
// to the main working tree's docs/Beaverton/ via git-common-dir.
function resolvePdfDir() {
  const candidate = path.join(ROOT, 'docs', 'Beaverton');
  ...
  try {
    ...
    const mainCandidate = path.join(mainRoot, 'docs', 'Beaverton');
...
function discoverPdfsByFY(pdfDir) {
  ...
  for (const f of files) {
    const m = f.match(/^beaverton-(\d{4})-acfr\.pdf$/i);
...
// (verified 2026-07-27).
const URLS = {
  2025: 'https://www.beavertonoregon.gov/asset/d2585702-6bab-43b0-9cfe-1171ab8577a1',
  2024: 'https://content.civicplus.com/api/assets/f663d61c-4d0e-47f0-8d35-bd5a9f378b05?scope=all',
...
function extractPDF(pdfPath, mode) {
  const pyScript = path.join(ROOT, 'scripts', 'extractBeaverton.py');
  ...
  if (result.status !== 0) {
    throw new Error(
      `extractBeaverton.py failed (exit ${result.status}) for ${pdfPath} [--mode ${mode}]: ` +
...
function dataSourceLabel(fy, datasetType) {
  ...
  return `City of Beaverton ACFR — General Fund ${kind} (FY${fy} actual, GAAP basis)`;
}

// ── Resolve Beaverton's municipality_id; refuse to write if not found ─────────────
async function ensureMunicipality(supabase) {
  ...
  if (error) { console.error('  ERROR resolving Beaverton municipality:', error.message); process.exit(2); }
  if (!data?.id) {
    console.error('  Beaverton, OR (entity_type=city) municipality not found — run ' +
                  'scripts/seedWashingtonCountyOregonCities.js first');
    process.exit(2);
  }
  console.log(`  Municipality: Beaverton, OR (${data.id})`);
  return data;
}

// ── Ephemeral data_sources lifecycle (WR-05/LOAD-01) ─────────────────────────
async function createEphemeralDataSource(supabase, muniId, datasetType) {
  const datasetId = datasetType === 'revenue' ? 'beaverton-acfr-gf-revenue' : 'beaverton-acfr-gf-operating';
  const kind = datasetType === 'revenue' ? 'Revenue' : 'Operating';
  const payload = {
    name: `Beaverton General Fund ${kind} Budget`,
    api_type: 'pdf_download',
    dataset_type: datasetType,
    dataset_id: datasetId,
    base_url: 'https://www.beavertonoregon.gov/financial-audits',
    fiscal_years: FYS,
...
async function processMode(supabase, muniId, dryRun, mode, targetFY, pdfsByFY) {
  ...
  try {
    for (const fy of years) {
      ...
      if (!pdfPath) throw new Error(`No PDF found for FY${fy} in docs/Beaverton/ — aborting`);
...
async function main() {
  ...
  if (!pdfsByFY.size) {
    console.error(`No Beaverton ACFR PDFs found in ${pdfDir}`);
    process.exit(1);
  }

  console.log(`Beaverton Budget Loader${dryRun ? ' (dry-run)' : ''} [${mode}]`);
