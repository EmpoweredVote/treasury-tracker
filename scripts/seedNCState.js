#!/usr/bin/env node
/**
 * North Carolina State Seeder
 *
 * Performs the following (all idempotent):
 *   A. Upsert North Carolina municipality row (entity_type='state', population=10439388,
 *      population_year=2024, state='NC')
 *   B. Upsert two data_source rows:
 *        - 'North Carolina General Fund Operating Budget'
 ...
 *        - 'North Carolina General Fund Revenue'
 ...
 *           fiscal_years=[2022,2023,2024,2025,2026])
 *   C. Verification: calls treasury_list_source_ids RPC and asserts sources appear.
 *      Exits non-zero if missing.
 *
 * North Carolina is a state-level entity (entity_type='state').
 * county_id stays NULL — states don't belong to a county.
 * Population: 2024 Census estimate (~10.44M NC total resident population).
 *
 * Usage:
 *   node scripts/seedNCState.js
 *
 * Env vars:
...
// ── Main ─────────────────────────────────────────────────────────────────────
async function main() {
  console.log('Seeding North Carolina state — municipality + data_source...\n');

  // ── Step A: Upsert North Carolina state municipality ──────────────────────
  console.log(`Upserting municipality: ${NORTH_CAROLINA.name}, ${NORTH_CAROLINA.state} (entity_type=${NORTH_CAROLINA.entity_type})`);
  const ncStateId = await upsertMunicipality(NORTH_CAROLINA);
  console.log(`  id: ${ncStateId}\n`);

  // ── Step B: Upsert data_source rows ─────────────────────────────────────
  const dataSources = DATA_SOURCE_TEMPLATES.map(t => ({ ...t, municipality_id: ncStateId }));
