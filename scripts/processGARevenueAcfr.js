#!/usr/bin/env node
/**
 * Georgia General Fund Revenue (by source) Loader — FY2021-FY2025 ACTUAL
...
const dataSource = (fy) => `Georgia State ACFR — General Fund Revenue (FY${fy} actual, GAAP basis)`;
...
function buildTree(fy, ex) {
  ...
  return { jsonTree: [{ n: 'Georgia General Fund Revenue', a: total * UNITS, c: children }], total: total * UNITS, rowCount: children.length };
}
async function main() {
  ...
  if (!dryRun) {
    ...
    const srcPayload = { name: 'Georgia General Fund Revenue', api_type: 'pdf_download', dataset_type: 'revenue', dataset_id: 'ga-acfr-gf-revenue', base_url: 'https://sao.georgia.gov/swar/acfr', fiscal_years: years, municipality_id: muniId };
