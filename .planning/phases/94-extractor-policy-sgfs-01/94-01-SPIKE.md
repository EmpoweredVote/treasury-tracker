...
## Build-time validation findings (Task 4, while proving on Georgia)
Three findings surfaced while building the loader and proving it on Georgia FY2023 — all consistent with the locked decision, recorded for Phases 95/96:
1. **Basis gap is small (~2%), not large.** NASBO MN GF FY2023 = $27.24B vs MN ACFR GAAP GF = $26.65B — a ~2% delta. So NASBO budgetary-GF and ACFR GAAP-GF are close in magnitude; the per-node basis label (P3) is a sufficient honesty mitigation (numbers are comparable, not wildly different). This strengthens the recommendation.
2. **NASBO has NO per-state revenue-by-source.** The Fiscal Survey reports GF revenue by source only at the NATIONAL aggregate (sales/PIT/CIT > 80%); states appear only in acknowledgments, not in a per-state revenue-by-source table. → **A per-state revenue-by-source tree cannot be built from NASBO.** Under the hybrid, **revenue-by-source DEFERS to the ACFR upgrade**; the NASBO loader supplies the operating (spending-by-function) tree now. (Total GF revenue per state is available, but a single-leaf "tree" isn't useful.)
3. **NASBO PDF text extraction misaligns** — blank/suppressed cells collapse and shift columns in `pdftotext` output (e.g. Georgia Table 1 text reads GF=$37,334M vs the visually-correct $27,657M). → Figures MUST be read from the rendered tables and **checksum-validated** (GF+Fed+Other+Bonds = row Total; 7-function GF sum ties to Table 1 GF). Georgia FY2023 proved this: 7 functions summed to $29.274B vs Table 1 GF $29.266B (0.03% tie). Phase 96's scaling parser must enforce these checksums per state-year.
