...
| **CO** | FY2023 (oldest in the confirmed 3-year window) | **$24,912,540K** | GF line items sum = $24,912,540K; matches printed Total Revenues. Diff = $0. ✅ |
...
| **CO** | `processPAAcfr.js` / `processPARevenueAcfr.js` (regular fund-column layout, but inconsistent per-year filenames requiring an explicit SOURCES map like IL/GA) | GF is 1st column of 4 (General Funds \| Federal Special Revenue Fund \| Highway Users Tax Fund \| Other Governmental Funds \| Total). Units = thousands. Column literally labeled "General Funds" (plural) — cosmetic naming quirk, same concept as every other state's "General Fund." | **P2 clamp required whenever the TABOR Excess Revenue line appears** (−$1,214,908K in FY2024; FY2023 netted the refund into tax revenue instead of a standalone line) — check for BOTH presentation forms every loaded year, not just a line labeled "TABOR." Requires a `Referer` header on every fetch (mild WAF) — no cookies needed, less severe than AZ. |
...
**Negative GF line items:** **CAUTION — FY2024 "TABOR Excess Revenue" = −$1,214,908K** in the General Funds column, a standalone negative revenue line for Colorado's constitutional Taxpayer's Bill of Rights refund mechanism. **FY2023 does NOT show this as a separate line** — the FY2023 narrative confirms TABOR refunds were accrued/paid that year too, but netted directly into the Individual Income Tax revenue line instead of broken out separately. This means the TABOR mechanism's *presentation* varies by year (sometimes a standalone negative line, sometimes netted into tax revenue) — Phase 113 must check both possibilities for every loaded year, not just look for a line labeled "TABOR."
