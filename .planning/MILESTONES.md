...
- **🔑 The NUL-byte lint is finally written, and it caught a fifth occurrence on its first run.** `.planning/STATE.md` held a raw `U+0000` — inside the line warning against NUL bytes — making git treat the file as binary. SCOPE-02 said this "wants a lint" after three occurrences; it is now `scripts/lib/nulByteLint.mjs` + `tests/nulByte.test.mjs`, enforced by `npm test` (a standalone script nobody runs is how the count reached four). ⚠ It immediately found **this milestone's own plan** committed as `Bin 0 → 86484 bytes` — an unreviewable binary blob in PR #31, again from a literal NUL inside a warning *about* NUL bytes. ⚠ Two traps recorded: the filter **cannot** ask git whether a file is binary (that is the symptom, so a corrupted `.ts` would exclude itself), and the first scan was **vacuous** — `LC_ALL=C grep -P '\x00'` errors on the locale and reports a clean repo it never read. Also: `sed -i` strips CRs and `perl`'s `s/\x00/\\0/` no-ops (octal NUL in the replacement).
