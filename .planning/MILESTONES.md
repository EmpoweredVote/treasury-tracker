...
- ⚠ **A fourth NUL byte.** `.planning/STATE.md` held a raw `U+0000` — inside the line warning against NUL bytes — making git treat the file as binary and destroying its diff and blame. Removed. SCOPE-02 said this "wants a lint" after three occurrences; **the lint is still unwritten.**
