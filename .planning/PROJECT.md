...
**Started:** 2026-07-16. **Shipped:** 2026-07-17 (all 9 requirements PIMA-01..09 complete; Phases 131-133 archived to `milestones/v2.18-phases/`). Reused the proven v2.17 Tucson playbook (`seedTucsonArizona.js` → `extractTucson.py` → `processTucson.js`, source-safe `treasury_sync_budget_tree`).
...
- ✓ **Oro Valley, Marana, Sahuarita, South Tucson onboarded at city parity** — recon + generalized `extractAcfrGF.py` (city-agnostic GF-column extractor), 44/44 dry-runs $0, South Tucson verdict = load-from-ACFR FY2019–2022 (PIMA-01/02/03) — v2.18, Phase 131
- ✓ 44 `budgets` rows loaded (OV/Marana/Sahuarita FY2019–2024, South Tucson FY2019–2022) via source-safe RPC (all tie $0, durably sourced, 0 residue, idempotent) + 4 munis seeded & linked to existing Pima node (Census Vintage-2024 pops 48,855/62,380/37,448/4,535) + 100% bleed-safe enrichment (PIMA-04/05/06) — v2.18, Phase 132
- ✓ Verification + live UAT: loader-independent re-derivation ties all 44 FY×mode + every category + every leaf at $0; source-chain audit clean (a–e); loader invariants confirmed + idempotent smoke-run clean; Essentials tether confirmed COVERED on all four banners (GEOIDs 0451600/0444270/0462140/0468850); Chris live-app UAT 34/34 (PIMA-07/08/09) — v2.18, Phase 133
...
---
*Last updated: 2026-07-17 after v2.18 milestone — v2.18 Pima County Municipalities — TT Budget Parity SHIPPED + archived (tag v2.18). Oro Valley, Marana, Sahuarita, South Tucson are live at city parity under the existing Pima County node from each town's own ACFR (GAAP): 44 `budgets` rows (OV/Marana/Sahuarita FY2019–2024, South Tucson FY2019–2022), Census-pinned populations (48,855 / 62,380 / 37,448 / 4,535), bleed-safe enriched, every figure durably sourced. Phase 131 recon + generalized `extractAcfrGF.py`; Phase 132 seed+load+enrich (44/44 $0, 100% enrichment); Phase 133 verified — loader-independent re-derivation ties all 44 FY×mode + every category + every leaf at exactly $0, source-chain audit clean (a–e, 0 residue, no stale labels), loader source-safety invariants confirmed + idempotent smoke-run clean, Essentials tether confirmed COVERED on all four banners (GEOIDs 0451600 / 0444270 / 0462140 / 0468850), Chris live UAT 34/34 all-pass. All 9 requirements (PIMA-01..09) complete. Code review found 2 Critical + 1 Warning latent-robustness findings in the verification harnesses (not the data) — all fixed (commit e9bd430), scripts re-run green, `133-REVIEW.md` resolved. Free ACFR PDFs only, $0 AI spend, executed via wave-based subagents. Phases 131-133 archived to `milestones/v2.18-phases/`. No active milestone. Next: `/gsd-new-milestone` (candidates: VOTES-01 votes/amendments hub; SRCSTD-01 sourced-standard city/state backfill). (v2.17 Tucson, AZ City Onboarding SHIPPED + archived 2026-07-11.)*
