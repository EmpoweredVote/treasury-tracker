...
**In progress — v2.12 State ACFR Long Tail.** Phase 105 complete (2026-06-30): **Pennsylvania** (FY2016–2025) + **Illinois** (FY2021–2025), the two largest remaining NASBO states, brought onto full State-ACFR GAAP — GF **revenue-by-source** + finer **spending-by-function** via four new loaders (`processPAAcfr.js`, `processPARevenueAcfr.js`, `processILAcfr.js`, `processILRevenueAcfr.js`) on the proven v2.11 pattern. NASBO operating replaced in place idempotently (RECON-05), scope-divergence accepted + relabelled honestly (PA ~2.0×, IL ~1.5× NASBO GF — TX-trap), P2 negative-category clamp on all four loaders (ACFR-08), Money In auto-enabled on both nodes. DB-verified: 30 rows live, 0 NASBO labels remaining on PA/IL, idempotent 0-row re-runs, CA/TX/NY/FL + GA/OH cohort untouched. Verified 6/6 (one in-phase gap closed: IL expenditure clamp). $0 spend. Remaining: Phase 106 (independent ACFR re-derivation + 50-node source-chain audit + Chris UAT) + deepening the 4 v2.11 pilots' history.
...
- [ ] **State ACFR Long Tail** — deepen the CA/TX/NY/FL windows (esp. FL pre-FY2022) + scale ACFR revenue-by-source to the remaining NASBO states
