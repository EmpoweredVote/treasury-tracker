...
| **TX** | FY2015–FY2024 (already contiguous) | **FY2015–FY2024 (no new work)** | 0 — **already loaded in v2.11** | FY2016 General Total Revenues **$96,239,551K** ✅ independently re-confirmed (matches the docs/ URL `processTX.js` already uses) | `…/2016/docs/96-471.pdf` (`docs/` infix) — **already in `processTX.js` SOURCES + EXPENDITURES** | thousands |
...
| TX | FY2016 | ALREADY DONE in v2.11 — `processTX.js` SOURCES already special-cases `…/2016/docs/96-471.pdf` and has a transcribed FY2016 EXPENDITURES block. The RECON-04 "TX FY2016" target was closed during the v2.11 loads. | Recon independently re-confirmed the source + GF total revenues tie ($96,239,551K), corroborating the existing loader. **No Phase-104 TX work.** |
...
- **TX — NO new work; already loaded in v2.11.** `processTX.js` already covers FY2015–FY2024 contiguously, including the FY2016 `docs/` special-case. Recon's independent re-confirmation of FY2016 ($96,239,551K General-column Total revenues, line-items + columns both tie) corroborates the existing loader. Column header is "General" (the consolidated General Revenue Fund concept, ~$96B — accept-relabel already applied in v2.11). FY-end Aug 31.
