...
- ▶ **v2.3 California Coverage Parity** — Phases 58-62 (in progress)

---

## ▶ v2.3 California Coverage Parity (Phases 58–62) — ACTIVE

**Milestone goal:** Bring every already-loaded CA city and county up to the Orange County standard — FY2003 budget-history depth, statewide salaries (2009–2024), and standardized enrichment — by re-running the hardened v2.2 SoCal pipeline. No new tooling. SoCal expansion (6 new counties, SOCAL-01..06) is deferred to v2.4.

**Pipeline reuse:** `bulkLoadStateController.js --county` (history), `loadCountyBudget.js` (county-gov budget), `loadCASalaries.js` (salaries), `seedCountyLinks.js` (linking), runbook `docs/socal-county-onboarding.md`. The never-overwrite guard protects the 12 named custom-source cities.

**Critical path:** 58 → 59 → 61 (enrichment needs categories loaded) → 62 (verify). 60 (salaries) is independent and can run in parallel with 58/59.
...
| 62 | ACFR Verification + Source-Chain Audit + UAT | 3/3 | Complete    | 2026-06-17 |

### Phase 58: LA County Parity Backfill

**Goal:** A citizen can view operating + revenue back to FY2003 for the 88 LA County cities AND for the LA County government itself — both from SCO ByTheNumbers, every figure sourced, per-year population for per-capita, with the 12 named custom-source cities left untouched.
**Depends on:** Nothing (reuses v2.2 `bulkLoadStateController.js` + `loadCountyBudget.js`)
**Requirements:** HIST-01, LAC-01
**Success Criteria** (what must be TRUE):

  1. Each of the 88 LA County cities (excluding the named custom-source cities, which are unchanged) shows operating + revenue reaching FY2003, every row carrying SCO ByTheNumbers source attribution and per-year population
  2. The LA County government page shows operating + revenue spanning FY2003–2024 (extending the existing FY2021–2025), all-governmental-funds basis documented, durable source rows
...
| 58-04 | App verification + per-capita spot-check | ✅ COMPLETE (2026-06-16) |
...
**Depends on:** Phase 58 (LA County backfill pattern + guard behavior confirmed first)
**Requirements:** HIST-02, ENR-02
**Success Criteria** (what must be TRUE):

  1. The other-county + SCO-covered unlinked CA cities show operating + revenue reaching FY2003 where the source provides it, every row sourced + per-year population
  2. The 1 currently budget-less CA city is loaded, or its absence is documented with a reason
  3. The 7 unlinked CA cities are linked via `county_id` to their correct county
  4. The breadcrumb chain (US → California → County → city) and Cities-in-County panel render for the newly linked cities in the live app

### Phase 60: Statewide CA Salaries Sweep

**Goal:** All non-OC CA cities carry CA Government Compensation salary data (2009–2024), reconciled on a sample.
**Depends on:** Nothing (reuses v2.2 `loadCASalaries.js`; independent of the history phases) — but salary state MUST be confirmed against the production/ev-accounts DB first (local shows only Bloomington IN)
**Requirements:** SAL-04, SAL-05, SAL-06
**Success Criteria** (what must be TRUE):

  1. Source coverage for the target CA cities is confirmed against GCC before any sweep writes (spike gate)
  2. Salaries 2009–2024 are loaded for the 88 LA County cities + the 12 named CA cities + other-county cities wherever GCC provides them
  3. A sample city's latest-year total compensation reconciles to a published figure at ~$0 delta
...
**Goal:** Parity data is independently reconciled against published ACFRs, the source chain is durable, and Chris signs off in the live app.
**Depends on:** Phases 58, 59, 60, 61 (all parity data + enrichment loaded)
**Requirements:** VER-03, VER-04
**Success Criteria** (what must be TRUE):

  1. LA County government + a representative sample of LA County cities reconcile against published ACFRs / adopted budgets on a basis-matched comparison, documented
  2. A source-chain audit passes — every backfilled budget/salary row carries durable source attribution (human-page URLs, no fragile/version-specific links), zero residue
  3. The live app is verified end-to-end: FY2003 history depth, salaries dataset, per-capita across backfilled years, enrichment, breadcrumbs + Cities-in-County panels
