...
- 🚧 **v2.14 State ACFR Long Tail — Tranche 3 + Deepening** — Phases 111-116 (in progress)
...
**Milestone goal:** Grow the State-ACFR GAAP cohort from 19 → ~29 states by bringing the **next ~10 largest-General-Fund NASBO states** (candidates: AZ, IN, CO, MO, KY, OR, SC, LA, OK, UT — recon locks the roster) onto full ACFR GF revenue-by-source + finer spending-by-function, and recover the **v2.13 history holes** (MA/CT/NJ modern-era gaps + CT/WI pre-GASB-34 via a new extractor) — with the **WR-05 loader-debt fix landing first** so every load this milestone runs residue-free.

**Constraints:** Free ACFR PDFs only ($0 / $5 AI gate); ACFR GAAP (GENERAL FUND column of the Governmental Funds Statement of Rev/Exp/Changes, `pdftotext -table`); every figure durably sourced + basis-labelled (incl. an honest pre-GASB-34 basis label); P2 negative-category clamp; idempotent never-overwrite (ACFR replaces NASBO per state-FY; the existing 19 ACFR nodes + un-upgraded NASBO states untouched); executed inline (no subagents); clone the proven per-state loader template (`process{PA,IL,NJ,...}{,Revenue}Acfr.js`); no frontend work — Money In + `?dataset=revenue` auto-enable once data lands.
...
1. Root cause of the recurring WR-05 residue (non-atomic / vestigial `data_sources` write in the shared loader pattern) is fixed at the template level, and the fix is applied to (or inherited by) the loaders that will run this milestone.
2. A live idempotency re-run of an existing loader (e.g. NJ, the 108-closure re-run precedent) leaves **0 residue rows with no manual re-clean** — proven by the cohort-audit residue probe run before and after.
3. Existing loaded data is unchanged after the re-run (never-overwrite intact; no row-count or total drift).

#### Phase 112: Recon — Roster Lock + Source Location + Overlap Resolution (RECON-09, RECON-10)

**Goal:** The ~10-state tranche-3 roster is locked from a NASBO-ranked shortlist with every locked state's ACFR GF statement located, tie-confirmed, and trap-checked — and every prior-load overlap resolved on paper before any write.
**Depends on:** Phase 111 (loads must not start before the loader fix; recon itself is read-only)
**Requirements:** RECON-09, RECON-10

Success criteria:

1. The remaining 31 NASBO states are ranked by GF size from NASBO 2025 SER and the ~10-state roster is locked (candidates AZ/IN/CO/MO/KY/OR/SC/LA/OK/UT; substitutions documented with reasons — rank correction or non-extractable ACFR).
...
**Requirements:** ACFR-21, ACFR-22, ACFR-23, ACFR-24, ACFR-25, ACFR-31, ACFR-32

Success criteria:

1. Each batch-1 state's node shows ACFR-sourced GF revenue-by-source + spending-by-function, basis-labelled, across its full recon-locked window; every loaded FY ties to its printed GF column totals.
2. NASBO operating rows replaced in place per state-FY; idempotent re-run writes 0 rows and leaves 0 data_sources residue (the Phase 111 fix holds under real use).
3. Scope divergence relabelled honestly where the ACFR GF is broader than NASBO's (ACFR-31); P2 clamp applied to any negative-category year with parent totals preserved (ACFR-32).
4. Money In auto-enables on each upgraded node (data-driven; no frontend change).

#### Phase 114: ACFR Upgrade — Batch 2 (~5 states) (ACFR-26..30 + ACFR-31/32)

**Goal:** The remaining ~5 roster states are upgraded NASBO→ACFR GAAP on the same standard as batch 1.
**Depends on:** Phase 112 (parallel with 113)
**Requirements:** ACFR-26, ACFR-27, ACFR-28, ACFR-29, ACFR-30, ACFR-31, ACFR-32

Success criteria:

1. Each batch-2 state's node shows ACFR-sourced GF revenue-by-source + spending-by-function, basis-labelled, across its full recon-locked window; every loaded FY ties to its printed GF column totals.
2. NASBO operating rows replaced in place per state-FY; idempotent re-run writes 0 rows and leaves 0 data_sources residue.
3. Scope divergence relabelled honestly (ACFR-31); P2 clamp applied where needed with parent totals preserved (ACFR-32); Money In auto-enabled.

#### Phase 115: Deepening — Recoverable Holes + Pre-GASB-34 Extractor (DEEP-02, DEEP-03, DEEP-04)

**Goal:** The v2.13 recoverable history holes are filled — modern-era gaps (MA FY2001/02/04/05/14/21, CT FY2006 OCR, NJ pre-FY2020) plus CT/WI pre-GASB-34 years via a new extractor with an honest basis label — or documented unrecoverable.
**Depends on:** Phase 111 (parallel with 113/114 — touches existing MA/CT/NJ/WI loaders, not the tranche)
**Requirements:** DEEP-02, DEEP-03, DEEP-04

Success criteria:

1. A pre-GASB-34 extractor handles the pre-FY2002 statement format, and every pre-GASB-34 row carries a distinct, honest basis label (visible in the app alongside the GAAP label).
2. MA FY2001/02/04/05/14/21, CT FY2006 (OCR path, free tooling), and NJ pre-FY2020 years are recovered with each year tying exactly to its printed GF total — or documented unrecoverable with the reason in the gap log.
3. CT/WI pre-GASB-34 years load via the new extractor as deep as durable URLs allow, each tying to its printed totals.
4. All deepening writes are idempotent + never-overwrite (existing modern-era rows untouched; re-run writes 0 rows).

#### Phase 116: Verification + Source-Chain Audit + UAT (VER-07, VER-08)

**Goal:** Every newly loaded state-FY is independently re-derived from its own ACFR, the full 50-state cohort audit stays clean with zero residue and no manual re-clean, and Chris signs off in the live app.
**Depends on:** Phases 113, 114, 115
**Requirements:** VER-07, VER-08

Success criteria:

1. Loader-independent blind re-derivation of every newly loaded state-FY (tranche 3 + deepening) at exact $0 delta against the printed ACFR GF totals — not loader self-report.
2. Full 50-state cohort source-chain audit clean: 0 NULL/fragile/residue/out-of-window/dup/orphan; every displayed row basis-labelled; un-upgraded NASBO states still pass; **0 data_sources residue with no manual re-clean** (LOAD-01 proven end-to-end).
3. Live-app UAT across a representative sample of upgraded states + deepened history years (revenue-by-source, spending-by-function, basis labels incl. pre-GASB-34, source chips, Money In, year selector reaching deepened years) — Chris sign-off.
