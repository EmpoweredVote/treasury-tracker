...
- ✅ **v2.23 WA-CITIES-01 — Six Largest WA Cities** — no GSD phases (shipped 2026-08-16, tag `v2.23` at `40aa706`) — 214 rows across Tacoma, Spokane, Vancouver, Bellevue, Kent, Everett + 4 nav-only county nodes; WA cohort now 286 rows; plan: `docs/superpowers/plans/2026-08-15-wa-cities-01.md`, closeout: `docs/superpowers/plans/WA-CITIES-01-CLOSEOUT.md`
- ✅ **v2.25 SCOPE-02 — Basis, Reporting Entity, and One Series per (scope, basis)** — no GSD phases (shipped 2026-08-18, tag `v2.25`) — `basis` + `reporting_entity` on `treasury.budgets`, the unique index widened so one city-year can hold two published figures, and **12 rows of SCO all-funds actuals backfilled that the old index kept out**; the seven CA seams no longer render as cliffs. PRs #16 and #17; plan: `docs/superpowers/plans/2026-08-17-scope-02.md`, closeout: `docs/superpowers/plans/SCOPE-02-CLOSEOUT.md`
- ✅ **v2.24 SCOPE-01 — Fund Scope on Every Row** — no GSD phases (shipped 2026-08-17, tag `v2.24`) — `treasury.budgets.fund_scope` app-wide: **53,404 rows classified from 8 evidenced sources, 26,523 (33.2%) honestly `unknown`**, zero figures moved; plan: `docs/superpowers/plans/2026-08-16-scope-01.md`, evidence: `docs/superpowers/plans/SCOPE-01-RECON.md`, closeout: `docs/superpowers/plans/SCOPE-01-CLOSEOUT.md`

v2.21 through v2.25 ran on `docs/superpowers/` specs + plans rather than `/gsd-plan-phase`, so they have no `.planning/phases/` directories and no `milestones/vX.Y-*` archives. Their unit of work is a numbered task in a plan document, not a phase. **GSD phase numbering therefore stops at 137 (v2.20); a future GSD-phased milestone continues from 138.** Full write-ups for all five are in `MILESTONES.md` — deliberately not duplicated here, because the detailed phase tables in this file are machine-parsed and a task table breaks that parse.

⚠ **v2.23's roadmap line said "awaiting Chris UAT before tag" until 2026-08-17, when the tag had in fact existed since 2026-08-16.** The same drift hit v2.21 and v2.22, which never reached this file at all. Tagging and updating `.planning/` are one step, not two.
...
### v2.19 Banner Info-Row + CTC Tether (Phase 134)

Adopted Essentials' `SectionBanner` info-row on the TT hero banner (left-anchored POPULATION stat + Essentials/CTC feature chips, top-left above the title) and added a per-location Civic Trivia Championship tether chip via a new `triviaCoverage` seam (mirror of `essentialsCoverage.ts`). Reciprocal of Essentials Ph187–189; extends v2.16 tethered icons. Frontend-only; verified (build/tests + Chris live UAT 6/6, 0 issues); deployed (bundle index-CNkPhEAJ.js).

- [x] Phase 134: Banner Info-Row + CTC Tether (1/1 plan, BANNER-01/CTC-01) — completed 2026-07-21
...
### v2.17 Tucson, AZ City Onboarding (Phases 128-130)
...
### v2.16 Tethered Icons & Smart Banner (Phases 125-127)
...
**Requirement coverage:** 12 requirements · 12 mapped · 0 unmapped ✓
...
**Goal:** Establish and consume the reciprocal coverage contract — Essentials publishes its coverage catalog as a build-time-generated static `coverage.json` (served at the Essentials origin with a CORS header for TT) **and** adds a national-officials browse route giving the federal entity a real target, and TT fetches + matches it, mirroring how Essentials calls TT's `/treasury/cities`.
**Requirements:** COV-01, COV-02, COV-03, COV-04
**Success criteria:**
1. Essentials serves its coverage catalog (cities/counties/all-50-states + a federal record, with Census GEOID + state + label + `hasContext`) as a generated static `coverage.json` at a public, unauthenticated URL with a CORS header allowing TT's origin — verified with a live cross-origin fetch.
2. TT fetches the catalog once per session and caches it in-memory; a slow, failed, or empty fetch never blocks or breaks the banner render (graceful degradation, async icon pop-in, no thrown error).
3. Given a TT entity, TT resolves it to an Essentials coverage record — tier-aligned — by name + state (city/county), state abbrev (state), or entity_type (federal), using loose matching (St./Saint, punctuation, trailing `County`/`, ST`), returning the GEOID(s)/abbrev/target or null (null on wrong/absent state).
...
5. Essentials serves a national-officials browse route (e.g. `/results?browse_federal_officials=1`) surfacing federal-tier officials (President/VP, U.S. Senate, U.S. House, Cabinet, Federal Judiciary, Independent Agencies), and the catalog's federal record targets it — TT's federal "United States" entity resolves to that real deep-link.
...
2. The Essentials icon deep-links the banner's current entity into Essentials (city/county → `browse_government_list`; state → `browse_state_officials`; federal → `browse_federal_officials`), opening in a new tab.
...
**Success criteria:**
1. A city/county with no Essentials coverage shows no Essentials icon; the federal entity **shows** the Essentials icon linking to the national-officials browse; a covered state/city shows the icon linking to the correct Essentials location.
...
**Milestone goal:** Finish the State-ACFR long tail — upgrade the last 21 NASBO states to full State-ACFR GAAP GF revenue-by-source + finer spending-by-function (all 50 states on ACFR), bundle the deepening holes on existing ACFR nodes (CA→FY2002, FL→FY2003), and retire NASBO to fallback-only. Cohort 29 ACFR → **50/50 ACFR**, 0 NASBO-served (2 honest fallback rows). Free PDFs only, $0 AI spend, executed inline.

- ✅ Phase 117: Recon — Source Location + Roster Lock + Overlap/Scope Pre-flight (RECON-11) — all 21 states located, bookend-tied at $0, roster locked, DEEP-05 URLs found; **zero STAY-NASBO** (verified 7/7, $0 DB writes)
- ✅ Phase 118: ACFR Upgrade Batch 1 — AK/AR/DE/HI/ID (ACFR-33..37) — all tie printed GF; AR single-fund honest basis note; ID FY2004 mixed-unit
- ✅ Phase 119: ACFR Upgrade Batch 2 — IA/KS/ME/MS/MT (ACFR-38..42) — all tie $0; ME June-30 FY-end confirmed
- ✅ Phase 120: ACFR Upgrade Batch 3 — NE/NV/NH/NM/ND (ACFR-43..47) — NM FY2022 raster-image OCR + P2 clamp; NV FY2024 honest NASBO fallback (ACFR ends FY2023)
- ✅ Phase 121: ACFR Upgrade Batch 4 — OK/RI/SD/VT/WV/WY (ACFR-48..53) — OK/SD scanned-image OCR; **WY last state → 50/50 ACFR**
- ✅ Phase 122: Deepening — Existing ACFR Node Pre-window Holes (DEEP-05) — CA +6→FY2002 (GASB-34 boundary), FL +18→FY2003–2020; NY/TX floors reconfirmed; FL FY2000–02 repair-pending
- ✅ Phase 123: NASBO Retirement (NASBORT-01) — NASBO path demoted to guarded fallback-only (`isAcfrOccupied`); 50/50-ACFR documented; no live node shows NASBO where ACFR exists
- ✅ Phase 124: Verification + Cohort Audit + UAT (VER-09, VER-10) — **149/151 blind re-derivation ties** (2 explained), **14/14 cohort invariants** over 1,560 rows (50/50-ACFR, NASBORT-01), **Chris UAT 12/12**

**Requirement coverage:** 26 v1 requirements · 26 mapped · 26 complete · 0 unmapped ✓
...
### v2.14 State ACFR Long Tail — Tranche 3 + Deepening (Phases 111-116)

**Milestone goal:** Grow the State-ACFR GAAP cohort from 19 → 29 states by bringing the next 10 largest-General-Fund NASBO states onto full ACFR GF revenue-by-source + finer spending-by-function, recover the v2.13 history holes via a new pre-GASB-34 extractor, and land the WR-05 loader-debt fix first so every load runs residue-free. Cohort now 29 ACFR + 21 NASBO = 901 rows, 0 anomalies. Free PDFs only, $0 AI spend.

- ✅ Phase 111: Loader Debt — Atomic data_sources Upsert (LOAD-01) — ephemeral data_sources lifecycle across all 35 loaders; live NJ re-run, 0 residue no manual re-clean (verified 3/3)
- ✅ Phase 112: Recon — Roster Lock + Source Location + Overlap (RECON-09/10) — ranked 31 NASBO states (0 drift), locked IN/AZ/OR/MO/CO ∥ SC/KY/UT/AL/LA (Oklahoma→Alabama rank-correction sub), bookend-tied at $0, overlaps resolved on paper, $0 DB writes (verified 16/16)
- ✅ Phase 113: ACFR Upgrade Batch 1 — IN/AZ/OR/MO/CO (ACFR-21..25, 31/32) — 68 state-years, all tie printed GF; CO TABOR P2 clamp; AZ FY2024 Drive-link caveat (verified 14/14)
- ✅ Phase 114: ACFR Upgrade Batch 2 — SC/KY/UT/AL/LA (ACFR-26..30, 31/32) — most FY2002–2025; GF-alone scope decisions honest (UT ~0.83×, AL ~0.24×, LA ~1.90×) (verified 8/8)
- ✅ Phase 115: Deepening — Pre-GASB-34 Extractor + Recoverable Holes (DEEP-02/03/04) — `pre34Extract.mjs`; CT 38yr contiguous (FY1988–2025, FY2006 via OCR), WI 26yr, NJ contiguous FY2002–2025, MA 2/6 recovered + 4 documented unrecoverable (verified 9/9)
- ✅ Phase 116: Verification + Source-Chain Audit + UAT (VER-07/08) — 75/75 blind re-derivation exact $0; 12-invariant cohort audit over 901 rows; LOAD-01 proven end-to-end (0 manual re-clean); Chris live UAT 11/11 all-pass 2026-07-03 (verified 3/3)
...
<details>
<summary>✅ v2.13 State ACFR Long Tail — Tranche 2 (Phases 107-110) — SHIPPED 2026-07-02 (full detail in milestones/v2.13-ROADMAP.md)</summary>

### v2.13 State ACFR Long Tail — Tranche 2 (Phases 107-110)

**Milestone goal:** Continue the proven State-ACFR GAAP upgrade (v2.11 CA/TX/NY/FL → v2.12 PA/IL) by bringing the **next 10 largest-General-Fund NASBO states** onto full ACFR GF revenue-by-source + finer spending-by-function, each as deep as durable ACFR URLs allow. NASBO operating replaced in place idempotently; un-upgraded states stay on NASBO. Cohort 9 ACFR nodes → 19.

- ✅ Phase 107: Recon — all 10 candidate states (NJ/MA/NC/GA/MD/TN/CT/WI/WA/MI) locked IN, 0 substitutions; bookend ties + durable per-year URLs + gap logs; MA in-place-upgrade path, GA F-97-01 supersede, NJ dollars-unit + MI Sep-30 FY-end traps caught pre-load (RECON-06/07)
- ✅ Phase 108: ACFR Upgrade Batch 1 — NJ, MA (in-place over v1.8 DLS), NC, GA, MD onto ACFR GAAP rev+spend; MD FY2022 P2 clamp; retroactive verification 4/4 + dedicated UAT 7/7 incl. live idempotency re-run (RECON-08, ACFR-09..13, ACFR-19/20)
- ✅ Phase 109: ACFR Upgrade Batch 2 — TN 17yr, CT 23yr, WI 24yr, WA 6yr, MI 7yr = 77 state-FYs; 6 live P2 clamps; scope divergence 1.14×–3.56× relabelled honestly (RECON-08, ACFR-14..18, ACFR-19/20)
- ✅ Phase 110: Verification — 49/49 loader-independent blind re-derivation exact $0; 50-state cohort audit 10/10 invariants (506 rows, 31 NASBO untouched); Chris live UAT 11/11 all-pass (VER-05/06)
...
### v2.12 State ACFR Long Tail (Phases 103-106)

**Milestone goal:** Extend the proven State-ACFR GAAP upgrade — deepen the four v2.11 pilots' (CA/TX/NY/FL) ACFR history backward, and bring **Pennsylvania + Illinois** (the two largest remaining NASBO states) onto full ACFR revenue-by-source + finer spending-by-function. Free PDFs only, `$0`/no paid AI, idempotent NASBO-replace, basis-labelled, executed inline.

- ✅ Phase 103: Recon — deeper-history URLs + PA/IL ACFR source location, bookend-tied + gap logs (RECON-04, RECON-05)
- ✅ Phase 104: Deepen the 4 pilots — CA FY2008–2025 (+12), NY FY2003–2014 (+12, ×millions), FL +FY2021 (P2 clamp); TX already contiguous (DEEP-01, RECON-05, ACFR-08)
- ✅ Phase 105: PA + IL ACFR upgrade — PA FY2016–2025 + IL FY2021–2025 onto full ACFR GAAP rev+spend, NASBO replaced idempotently, scope divergence relabelled honestly, IL FY2022 clamp (ACFR-06/07/08, RECON-05)
- ✅ Phase 106: Verification + source-chain audit + UAT — 24/24 loader-independent re-derivation exact ties, 50-node cohort audit 7/7 (276 rows, 41 NASBO untouched), Chris live UAT 8/8; in-milestone UI fix (distinct colors + dropped redundant single-root layer) deployed (VER-03, VER-04)
