...
status: "v2.30 SCOPE-04 SHIPPED — merged `d1e77c3` (PR #52), tagged `v2.30`, and ✅ **UAT RUN AND PASSED 2026-08-23, 8 of 10 tests** (`docs/superpowers/plans/SCOPE-04-UAT.md`). 7,650 derived Total Governmental rows across 488 CA entities. ⚠ UAT still outstanding on v2.25, v2.26, v2.27 and v2.29."
...
**Current focus:** **v2.30 SCOPE-04 is SHIPPED and UAT-VERIFIED (2026-08-23).** Read `docs/superpowers/plans/SCOPE-04-UAT.md` first — it is the record of six defects the UAT found, five of them fixed in **PR #54, which is still unmerged**. Then `SCOPE-04-CLOSEOUT.md` and `SCOPE-04-RECON.md` for the milestone itself. Nothing new should touch `src/` before #54 is reviewed.

## Current Position

Milestone: **v2.30 SCOPE-04 — Derived Total Governmental + the enterprise slice — ✅ SHIPPED 2026-08-22, merged `d1e77c3` (PR #52), tag `v2.30`, ✅ UAT-VERIFIED 2026-08-23**
Task: milestone on a `docs/superpowers/` plan, not GSD phases — see the banner above
Status: **All 13 tasks executed. 7,650 derived rows live in the database.** Gates at close: `npm test` **552 passed / 35 files**, `npm run build` clean, `acfrGF.selftest.py` **166 passed**, `verify-fund-scope.mjs` **all checks passed**, `verify-budget-axes.mjs` **all checks passed**. ✅ **UAT run 2026-08-23 — 8 of 10 passed**, `docs/superpowers/plans/SCOPE-04-UAT.md`.
...
✅ **UAT RUN AND PASSED 2026-08-23 — 8 of 10 tests.** Record: `docs/superpowers/plans/SCOPE-04-UAT.md`. Deployment was verified live *before* testing rather than assumed. Every claim the milestone makes on screen holds — the toggle moves the figures to the dollar, the derived series names itself, the disclosure renders beside the derived figure and only there, the enterprise slice is visible as categories appearing and disappearing, Napa FY2017 reads $97,734,023, and nothing outside California moved.

⚠ **SIX DEFECTS FOUND — five fixed test-first in PR #54, which is UNMERGED.** Not one was SCOPE-04's own work: all six were pre-existing paths that SCOPE-04 made *reachable* by giving 488 CA entities a second series. G1 arriving on an Employees link blanked both budget tiles (**480 CA entities**); G6 a stale load stamped its state over a newer one; G5 the year-clamp note had never rendered once; G2 a leaf click dimmed every icicle row with nothing clickable; G3 a drilled level was one flat colour; **G4 is open by decision** — drilling hides the dataset tabs and the series pills, so the breadcrumb is the only way back.

⚠ **No arithmetic gate could have found any of the six.** Every figure involved was correct. What was wrong was *which* figure was on screen, whether the reader was told, and whether the thing they clicked did anything. Three of the five fixes had to move logic out of components into pure modules first, because this repo can run **no component tests** — see `reference_no_component_tests_vitest`.

⚠ **UAT is still outstanding on four milestones** — v2.25, v2.26, v2.27 and v2.29.

**Next: merge PR #54, then the four-milestone UAT backlog and the successor-agency magnitude question.** The backlog (v2.25–v2.29) is still the largest this project has carried. The 15 unread SCOPE-04 sample targets are committed and should be read for successor-agency magnitude rather than for a tie — that is the number nobody has.
