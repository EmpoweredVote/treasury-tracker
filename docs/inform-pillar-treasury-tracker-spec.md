<!--
────────────────────────────────────────────────────────────────────────────────
PROVENANCE — this block is the ONLY editorial addition. Everything below the
`---` marker is the original file, byte for byte.

Rescued from the orphan `master` branch before that branch was deleted.

  origin/master tip   f3f50101d83ffbb9402be3ee77f10998fd47048f
  original path       3_Inform_Treasury-Tracker-detailed.md (repository root)
  original size       40,385 bytes
  original sha256     37c696da3f480881178ae4fcce74a0d9d3918ec7af84aa5c97e359d2ad5a7db8

⚠ `master` shared NO COMMIT with `main` — `git merge-base` exited 1. It was a
35-commit parallel history, all authored on 2026-03-21, building a "milestone
v1.1 Multi-City Platform" around Bloomington, Indiana. That work was rebuilt on
`main` rather than merged, so none of it was reachable from the shipped app and
this document existed in exactly one place.

⚠ THIS IS A HISTORICAL PRODUCT BRIEF, NOT A DESCRIPTION OF THE CURRENT APP. It
predates essentially all of Treasury Tracker as it now stands, and it names
Bloomington 31 times because Bloomington was the prototype city. Read it for
intent, never as a spec of present behaviour.

Roughly where it stands against what shipped, as of 2026-09-04:

  SHIPPED    the two view modes, the Money Out / Money In split (Money In is
             live behind `?dataset=revenue`), drill-down to line items, source
             attribution, and the deep links to Empowered Essentials
  PARTIAL    the People / workforce view — salaries data exists, but as a
             two-level tree rather than the shape described here
  NOT BUILT  the "Follow Your Tax Dollars" personal contribution calculator,
             the "Did You Know?" educational pop-ups, and the Compass /
             Read & Rank / Issues in Focus integrations

The full branch, including ~29 raw Indiana AFR source files and the Indiana
file-layout spreadsheets that also existed nowhere else, was bundled before
deletion — see the commit message for where.
────────────────────────────────────────────────────────────────────────────────
-->
