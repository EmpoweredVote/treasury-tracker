...
// Above this count, show a filter box + scroll container (MA has 350+ cities).
const FILTER_THRESHOLD = 24;

/**
 * City tags shown at the bottom of a state page — the state's jump-off point to
 * its municipalities, mirroring CitiesInCountyPanel and StatesInFederalPanel.
 * Includes every sub-state government entity in the state (city, town, township,
 * municipality, special district, …); counties get their own breadcrumb level.
 */
        ...
        <div className={withoutData.length > 0 ? 'mb-6' : undefined}>
            ...
            {showFilter && (
              <div className="relative w-full sm:w-56">
                ...
                <input
                  ...
                  placeholder={`Filter ${withData.length} cities…`}
                  aria-label={`Filter cities in ${state.name}`}
          ...
          <div className={`flex flex-wrap gap-2 ${showFilter ? 'max-h-72 overflow-y-auto pr-1' : ''}`}>
            ...
            {showFilter && filter && filtered.length === 0 && (
              <p className="text-sm text-ev-gray-500 py-1">No cities match "{filter}".</p>
            )}
          </div>
        </div>
      )}
